...
const createTasks = (title, freq, start, end, startDate, endDate) => {
    const uniqueId = () => {
...
const RecurringForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const tasks = createTasks(title, freq.value, start, end, startDate, endDate);
    ...
    <form onSubmit={handleSubmit} className='task-form-recurring'>
        ...
        <Select className='freq' options={options} closeMenuOnSelect={true} onChange={setFreq} isSearchable={false}/>
        <label>Time</label>
